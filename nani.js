...
let embed = new MessageEmbed().setColor("RANDOM").setTimestamp().setFooter("Arcenio ❤️ Valentina").setTitle("Rol Koruma")
       ...
       //Rol Silinme Koruması
        bot.on("roleDelete", async(deletedRole) => {
            ...
            if (entry && ![deletedRole.guild.ownerID, ...ayar.whitelistMembers].includes(entry.executor.id)) {
                ...
                yetkili.ban({ reason: "Rol Silme - Arcenio Guard" })
            }
        });
    })
});
...
client.on("ready", () => {
    client.channels.cache.get("914513781890113541").join();
    });